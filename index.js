...
const N8N_WEBHOOK_URL =
  ...
  "https://n8n-vwjn.onrender.com/webhook-test/baileys";
const BAILEYS_BASE_URL =
  process.env.BAILEYS_BASE_URL || "https://baileys-bridge.onrender.com";
...
async function connectToWhatsApp() {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const message of messages) {
      ...
      const contact =
        message.pushName || message.key.remoteJid?.split("@")[0] || "Unknown";
      const phoneNumber = message.key.remoteJid?.includes("@s.whatsapp.net")
        ? message.key.remoteJid.split("@")[0]
        : message.key.remoteJid;

      // Send only the fields n8n expects
      const messageData = {
  ...
  try {
    ...
    switch (type) {
      ...
      case "image":
        ...
        return res
          .status(400)
          .json({ error: "Image messages not implemented yet" });
      default:
